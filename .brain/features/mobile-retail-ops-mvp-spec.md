...
- Add variant now defaults to an inline keyboard-sticky composer: it first shows filtered variant-label chips above a chat-style input, then stays open to collect values for the selected label. Existing values appear first as removable chips, common values are mapped from the selected label, value entry hides the plus action and accepts comma-separated or Enter-submitted values, the composer closes when the keyboard is dismissed or the user taps back into the form, and the older modal label/value sheets remain in code as a rollback path while the inline composer is tested.
...
- The Stocks tab keeps the existing generated sellable rows and their price, stock, and optional media fields. Row labels are `Price` and `Stock`; the row action offers Enable/Disable instead of delete. Disabled stock rows and rows inheriting a disabled variant value sort after active rows without losing their own stock-level enabled state.
