...
After login, a user can turn on a device-local App lock from mobile Settings. The lock stores only local secure-state on the phone, uses a 6-digit PIN, can optionally call the phone's fingerprint/biometric prompt, and appears whenever the authenticated app is reopened or returns from background. This is not a replacement for account login; forgotten PIN recovery signs the user out, clears the local app lock for that user/device, and requires normal login again. App-lock hydration fails closed with a bounded secure-storage read: when device storage rejects or stops responding, the screen leaves its checking state, keeps PIN entry disabled, explains the storage failure, and preserves a responsive sign-out/reset recovery path.
