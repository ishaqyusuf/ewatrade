...
- OTP entry should feel like a compact PIN verification surface: separated display cells, in-screen numeric keypad, clipboard paste support, resend state, local fallback note for dev/test paths, and auto-submit when the six-digit code is complete.
- OTP emails should use the shared email/notification/job packages rather than screen-local fetch logic. In local/dev environments, or for exact `@test.com` submitted emails, outbound OTP delivery routes to `TEST_EMAILS` or the legacy `TEST_EMAIL` fallback while preserving the submitted account email for verification.
