...
- OTP entry should feel like a compact reference-led PIN verification surface: a minimal off-white/light or matte dark canvas, circular back action, centered headline/copy, separated display cells, in-screen rounded numeric keypad with phone-letter hints, clipboard paste/delete support, resend state, local fallback note for dev/test paths, and auto-submit when the six-digit code is complete.
...
- Development mobile login uses the reusable `FeatureFlag` wrapper to replace the real email-code form with a floating bottom-sheet business picker. Selecting a local business routes directly to the OTP screen through local fallback, and when `SKIP_OTP=1` the app/API use the fixed development code `123456`, skip real email dispatch, and show a `use 123456` toast on the OTP screen.
