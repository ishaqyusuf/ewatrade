...
- The center `+` always opens a compact Create sheet for owner/admin users instead of bypassing the chooser based on catalog state. The sheet includes contextual Order access when catalog items exist, plus Item, Customer, Stock entry when Products exist, and Sales rep actions.
