...
- The center `+` always opens a compact detached floating Create sheet for owner/admin users instead of bypassing the chooser based on catalog state. The headerless, flat, divider-based sheet uses one consistent plus icon and exposes separate `Product` and `Service` actions that preselect the generic catalog item kind, `Customer`, `Order`, `Stock Entry`, and `Staff`. `Order` remains visible but disabled until an active fixed-price Product or Service offering is available at a store, with guidance to create the first Product/Service. `Stock Entry` remains visible but disabled until the workspace has a Product, so Service-only businesses are not sent into inventory workflows. Sales reps retain the fastest direct `New order` center action.
...
- Keyboard-sticky inline composers should sit as accessory surfaces directly above the keyboard only while activated, with horizontal suggestion chips above the input, a single right action when the mode needs explicit completion, and removable selected chips for already-added values. Variant-value entry uses a checkmark action that commits any unfinished value and closes the composer, while commas may commit completed values without closing it and keyboard Done preserves the same completion behavior. The completion action should remain enabled after a suggestion is selected. Composers should also close when the keyboard is dismissed or the user taps back into the form. Development-only floating controls should hide while the keyboard is visible so they do not cover composer pills or inputs.
...
- Variant entry should default to an inline keyboard-sticky composer rather than a modal: a horizontal pill rail sits above a chat-style input with one right-side action. The composer starts in variant-label mode with a plus action and filtered known labels, then stays open in value-entry mode with a checkmark completion action, selected removable chips first, and mapped common-value suggestions after them. The checkmark commits unfinished text, stays available after suggestion selection, and closes the composer. The older compact value sheet can remain in code as a rollback path during testing.
- Selling-unit setup stays visible by default as a compact `Unit` section. The
  section explains common examples, places `Add unit` before the list, and
  renders saved units as flat divider rows with 44pt edit/delete controls. Unit
  creation and editing use the same detached keyboard-aware bottom sheet with
  concrete examples for the name and exact conversion factor. Quantity
  precision is hidden from this setup surface and new units allow up to two
  decimal places by default.
- Generated Product variant rows expose one counted-unit price plus one
