...
- Mobile auth contracts:
  ...
  - In non-production environments only, `SKIP_OTP=1` makes `auth.requestMobileOwnerOtp` write the fixed development code `123456` and skip outbound email dispatch. Production ignores this flag even if it is present.
