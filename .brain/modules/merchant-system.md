...
- generates a one-time invite acceptance token, stores only its hash, revokes prior active tokens for the same membership, and keeps the raw token only long enough to build the invite URL
- resolves public invite tokens into bounded business/email/role context without exposing the secret token
...
- mirrors invite, onboarding, status, profile, and lifecycle-audit data into durable staff tables when the migration is available
...
This phase still needs live email-provider configuration, durable stock wallet table cutover, ledger-backed custody movements, return approval workflow, richer reconciliation, and subscription staff usage derived fully from durable snapshots.
...
- resolves first-time customer registration and returning login through Better Auth before order request submission; raw passwords are not stored in order data or metadata
...
- records platform customer account identity in the order request and customer book when the customer registers or logs in through the shared-link checkout
...
- lists durable product links for the selected tenant/store first, with metadata-backed product links as rollout fallback
- returns generated-link analytics from daily rollup rows when available, with generated-link counters as rollout fallback
...
This phase still needs live email-provider configuration, provider-native webhooks, provider-native online payment from share links, provider bidding/selection, richer driver assignment UI, richer tracking views, and richer customer account onboarding beyond direct shared-link checkout. The first public link surface remains web-based; native customer browsing and checkout are a later extension.
