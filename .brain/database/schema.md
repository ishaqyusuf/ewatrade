...
- `retailOps.unitTemplates` reads active durable system and tenant `ProductUnitTemplate` rows with ordered `ProductUnitTemplateUnit` ratios when available, then merges fallback bag-fraction, 25 kg feed-bag, 50 kg feed-bag, and kilogram-fraction presets so setup assistance works before seeds or migrations are deployed.
