...
- [x] Runtime Service and Prescription paths read and write only the Commerce-owned Quote aggregate.
...
The Neon development schema is current and the idempotent backfill reported a
zero-row result twice. Production reconciliation and contraction remain
blocked and are not implied by the empty development dataset.
