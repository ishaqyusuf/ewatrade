...
for (const theme of ["light", "dark"]) {
  for (const [key] of names) {
    ...
    const layout = await page.evaluate(() => {
      ...
      return { activeCount: active.length, phoneWidth: phone.width, phoneHeight: phone.height, statusTop: status.top - phone.top, navBottom: nav.bottom - phone.top }
    })
...
if ((await board.textContent("#current-name")) !== "Cashbook Columns") throw new Error("Previous chevron did not wrap from A to E")
await board.click("#next")
await board.click("#next")
if ((await board.textContent("#current-name")) !== "Fulfilment Board") throw new Error("Next chevron did not advance from A to B")
