...
- Opening stock, stock intake, and unit conversion do not yet persist dedicated ledger rows.
...
- Invite email delivery, acceptance tokens, subscription staff limits, stock wallets, and custody assignment remain future production slices.
...
- Opening inventory confirmation lines are not yet persisted server-side.
...
- Stock wallets, customer-book persistence, ledger movements, idempotent offline replay, and closeout linkage remain future production slices.
...
- `retailOps.createProductShareLink` creates an opaque web-first product URL for an existing product.
...
- Link records are metadata-backed for now. Durable source schema exists for link records, events, views, order requests, reservations, notification audit, and daily analytics rollups; customer auth/password repository wiring, real email provider wiring, live stock reservation, payment, fulfillment, and durable analytics reads remain future production slices.
