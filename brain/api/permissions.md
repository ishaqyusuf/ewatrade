...
- First-phase staff management does not yet enforce secure acceptance tokens, real email provider configuration, durable staff profiles, or stock return approval workflow.
...
- The MVP web checkout requires first-time customer registration and returning-customer login before order request submission; the current bridge may accept explicit customer details until durable customer auth is wired.
- First-phase share-link APIs do not yet configure a real email provider, reserve stock, accept payment, fulfill orders, or enforce subscription campaign limits.
