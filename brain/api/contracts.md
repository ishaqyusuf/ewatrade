...
- Retail Ops staff management contract:
  ...
  - This is a membership, list, first email, staff-limit, account-based onboarding, and metadata-backed stock-wallet bridge, not the final invitation/custody lifecycle. Secure acceptance tokens, real email provider configuration, staff profile details, durable stock wallet tables, ledger-backed assignment/return movements, return approval workflow, and reconciliation remain required.
...
- Retail Ops product setup contract:
  - `retailOps.createProduct` accepts optional `storeId`, optional `description`, optional `externalId`, optional `unitTemplateKey`, product `name`, `primaryUnitName`, primary `priceMinor`, primary `openingStockQuantity`, and up to 24 variant units
  - Product setup is restricted to owner/admin/manager-level sales-management roles
  - Each variant unit accepts `name`, `priceMinor`, optional `openingStockQuantity`, and optional `conversionMultiplier`
