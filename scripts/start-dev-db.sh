...
COMPOSE_FILE="${EWATRADE_COMPOSE_FILE:-$ROOT_DIR/compose.yaml}"
SERVICE_NAME="${EWATRADE_DB_SERVICE:-postgres}"
DB_HOST_PORT="${POSTGRES_PORT:-5432}"
DB_USER="${POSTGRES_USER:-postgres}"
DB_NAME="${POSTGRES_DB:-ewatrade}"
MAX_ATTEMPTS="${EWATRADE_DB_WAIT_ATTEMPTS:-30}"
SLEEP_SECONDS="${EWATRADE_DB_WAIT_SECONDS:-1}"
DOCKER_MAX_ATTEMPTS="${EWATRADE_DOCKER_WAIT_ATTEMPTS:-60}"
DOCKER_SLEEP_SECONDS="${EWATRADE_DOCKER_WAIT_SECONDS:-2}"
...
Your .env DATABASE_URL should use localhost:$DB_HOST_PORT, so the app would connect to whichever process owns that port instead of this container.
EOF
  print_port_diagnostics
  return 1
}
