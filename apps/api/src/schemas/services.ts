...
const serviceQuoteLineSchema = z
  .object({
    offeringId: z.string().trim().min(1),
    quantity: exactServiceQuantitySchema,
...
const serviceQuoteOptionSchema = z
  .object({
    ...
    lines: z.array(serviceQuoteLineSchema).min(1).max(100),
...
export const serviceQuoteIssueSchema = z
  .object({
    ...
    discountMinor: z.number().int().nonnegative().optional(),
    expiresAt: z.coerce.date().optional(),
    lines: z.array(serviceQuoteLineSchema).min(1).max(100).optional(),
    options: z.array(serviceQuoteOptionSchema).min(1).max(20).optional(),
    requestId: z.string().trim().min(1),
    storeId: z.string().trim().min(1).optional(),
    taxMinor: z.number().int().nonnegative().optional(),
  })
  ...
  .superRefine((value, context) => {
    if ((value.lines ? 1 : 0) + (value.options ? 1 : 0) !== 1) {
      context.addIssue({
        code: "custom",
        message: "Use either Quote lines or Offer Options.",
        path: ["options"],
      })
    }
    if (
      value.options &&
      (value.discountMinor !== undefined || value.taxMinor !== undefined)
    ) {
      context.addIssue({
        code: "custom",
        message: "Offer Options own their monetary adjustments.",
        path: ["options"],
      })
    }
  })
...
export const publicServiceQuoteOptionSelectSchema = z
  .object({
    acceptanceToken: z.string().trim().min(20).max(500),
