...
export function CloseoutContent({
  ...
  const sourceDetail = isOfflineMode
    ? "This closeout will sync when connection is ready."
    : openSession?.remoteId
      ? pendingSyncCount > 0
      ...
      closeSessionMutation.mutate(
        {
          ...
          inventoryLines: inventoryLines.map((line) => ({
            countedQuantity: parseWholeQuantity(inventoryDraft[line.id] ?? "0"),
    ...
    <View className={contentClassName}>
      <View className="gap-2">
        ...
        <Text className="text-sm leading-5 text-muted-foreground">
          Confirm payment declarations and closing stock before admin review.
        </Text>
      </View>
      ...
      <View className="gap-3">
        ...
        {inventoryLines.length > 0 ? (
          <View className="gap-3">
            ...
            {filteredInventoryLines.length > visibleInventoryLines.length ? (
