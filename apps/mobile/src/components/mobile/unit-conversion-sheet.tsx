...
export function UnitConversionContent({
  ...
  const source = balances.data?.rows.find(
    (row) => row.balanceSourceId === sourceId,
  )
  const target = balances.data?.rows.find(
    (row) => row.balanceSourceId === targetId,
  )
  ...
  const submit = () => {
    if (offline) {
      setError("Packaged-stock transformation requires a connection.")
      return
    }
    if (
      !source ||
      ...
      setError(
        "Choose source and target balances, enter both quantities, and add a reason.",
      )
      return
    }
    mutation.mutate({
      clientOperationId: `transform-${Crypto.randomUUID()}`,
      ...
      sourceQuantity: sourceQuantity.trim(),
  ...
  return (
    ...
    >
      {offline ? (
        <StatusBanner
          icon="Wind"
          message="Connect before transforming packaged stock."
          title="Online action"
          tone="warning"
        />
      ) : null}
      ...
      <Text className="font-extrabold text-foreground">Source balance</Text>
      <View className="gap-2">
        {rows.map((row) => (
          <BalanceChoice
            active={sourceId === row.balanceSourceId}
            key={row.balanceSourceId}
            label={`${row.productName} · ${row.variantName} · ${row.inventoryUnitName}`}
            onPress={() => setSourceId(row.balanceSourceId)}
      ...
      <Text className="font-extrabold text-foreground">
        Target packaged balance
      </Text>
      <View className="gap-2">
        {rows
          .filter(
            (row) =>
              row.kind === "PACKAGED_STOCK" && row.balanceSourceId !== sourceId,
          )
          .map((row) => (
            <BalanceChoice
              active={targetId === row.balanceSourceId}
              key={row.balanceSourceId}
              label={`${row.productName} · ${row.variantName} · ${row.inventoryUnitName}`}
              onPress={() => setTargetId(row.balanceSourceId)}
              quantity={row.onHandQuantity}
            />
          ))}
      </View>
      ...
      <FormField
        ...
        value={reason}
      />
      <ActionButton isLoading={mutation.isPending} onPress={submit}>
...
function BalanceChoice({
  ...
  onPress,
  quantity,
}: { active: boolean; label: string; onPress: () => void; quantity: string }) {
  return (
    <Pressable
