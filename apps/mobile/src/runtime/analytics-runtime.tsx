...
export function AnalyticsRuntime() {
  ...
  const route = "/" + segments.filter((segment) => !segment.startsWith("(") && !segment.startsWith("[")).join("/")
  ...
  useEffect(() => {
    ...
    const analytics = createNativeAnalytics({ endpoint, enabled: true, createId: randomUUID, storage: {
      getItem: (key) => SecureStore.getItem(keyFor(key)),
      setItem: (key,value) => SecureStore.setItem(keyFor(key),value),
      removeItem: (key) => { void SecureStore.deleteItemAsync(keyFor(key)).catch(() => {}) },
    }})
    client.current = analytics
    analytics.init()
    analytics.trackPageView({route:latestRoute.current})
    const listener = AppState.addEventListener("change", (state) => { if (state === "active") analytics.trackPageView({ route: latestRoute.current }); void analytics.flush() })
    return () => { listener.remove(); void analytics.flush().finally(()=>analytics.destroy()); client.current = null }
  }, [])
  useEffect(() => { client.current?.trackPageView({route}) }, [route])
  return null
}
