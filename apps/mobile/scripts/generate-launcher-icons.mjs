...
const OUTPUTS = [
  ...
  { file: "dev-adaptive-icon.png", transparent: false },
  { file: "loading-icon.png", transparent: false },
  { file: "dev-loading-icon.png", transparent: false },
  { file: "ios-light.png", transparent: false },
  { file: "dev-ios-light.png", transparent: false },
  { file: "ios-dark.png", transparent: false },
  { file: "dev-ios-dark.png", transparent: false },
];
...
function composeIcon(mark, { transparent }) {
  ...
  const offsetX = Math.round((CANVAS_SIZE - mark.width) / 2);
  const offsetY = Math.round((CANVAS_SIZE - mark.height) / 2);

  for (let y = 0; y < mark.height; y += 1) {
    for (let x = 0; x < mark.width; x += 1) {
      const source = (y * mark.width + x) * 4;
      const target = ((offsetY + y) * CANVAS_SIZE + offsetX + x) * 4;
      const alpha = mark.data[source + 3] / 255;
      const inverseAlpha = 1 - alpha;

      data[target] = Math.round(mark.data[source] * alpha + data[target] * inverseAlpha);
  ...
  return { width: CANVAS_SIZE, height: CANVAS_SIZE, data };
}
...
const targetTileSize = Math.round(
  (TARGET_ART_WIDTH / sourceMeasurement.width) * source.width,
);
const resizedMark = resizePremultiplied(source, targetTileSize, targetTileSize);

for (const output of OUTPUTS) {
  const image = composeIcon(resizedMark, output);
  ...
  console.log(
    `${output.file}: ${CANVAS_SIZE}x${CANVAS_SIZE}, artwork ${measurement.width}x${measurement.height}`,
  );
}
