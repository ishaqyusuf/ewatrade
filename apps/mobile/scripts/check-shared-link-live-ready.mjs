...
for (const group of URL_GROUPS) {
  ...
  validateUrlGroup({
    allowLocalhost: env.SHARED_LINK_LIVE_ALLOW_LOCALHOST === "1",
    key: selectedKey,
...
const followUpMode = env.SHARED_LINK_LIVE_FOLLOW_UP_MODE?.trim()
if (!["cancel", "complete"].includes(followUpMode)) {
  failures.push(
    "SHARED_LINK_LIVE_FOLLOW_UP_MODE must be either complete or cancel so live QA does not leave an ambiguous pending order request.",
  )
}

const quantity = env.SHARED_LINK_LIVE_QUANTITY?.trim()
if (quantity && (!/^\d+$/.test(quantity) || Number(quantity) < 1)) {
  failures.push(
    "SHARED_LINK_LIVE_QUANTITY must be a positive whole number when set.",
  )
}
...
function validateUrlGroup({ allowLocalhost, key, value }) {
  ...
  if (isLocalhost && !allowLocalhost) {
    failures.push(
      `${key} points to a local host. Set SHARED_LINK_LIVE_ALLOW_LOCALHOST=1 only for an intentional local live-QA rehearsal; production/database-write QA should use deployed URLs.`,
    )
  }
}

function validateEvidencePath() {
  const evidencePath = env.SHARED_LINK_LIVE_EVIDENCE_PATH?.trim()

  if (!evidencePath) return

  if (!isAbsolute(evidencePath)) {
    failures.push("SHARED_LINK_LIVE_EVIDENCE_PATH must be an absolute path.")
  }

  if (!evidencePath.toLowerCase().endsWith(".json")) {
    failures.push("SHARED_LINK_LIVE_EVIDENCE_PATH must point to a .json file.")
  }
}
