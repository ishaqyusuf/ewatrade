...
const FILES = {
  ...
  sessionFlow: join(MOBILE_DIR, "src/components/mobile/session-flow.tsx"),
  sessionsRouter: join(
    REPO_ROOT,
...
const CONTRACTS = [
  ...
  },
  {
    ...
    reason:
      "rep-session and closeout screens must use shared flat reconciliation primitives with semantic status tokens instead of local card-heavy session widgets",
  },
  {
  ...
  },
  {
    ...
    markers: [
      ...
      "conversionPreview.targetQuantity",
      "mobile:unit_conversion:",
      "Crypto.randomUUID()",
  ...
  },
  {
    ...
    reason:
      "inventory product, unit, segment, and movement rows must use reusable flat no-image primitives with selection, stock, price/status, and divider treatment",
  },
  {
    file: FILES.store,
    markers: [
  ...
  },
  {
    ...
    reason:
      "schemas must keep idempotent, whole-number inventory/session contracts while allowing the server to derive conversion output",
  },
  {
