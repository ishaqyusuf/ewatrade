...
const SCRIPT_DIR =
  process.env.MVP_LIVE_READINESS_SCRIPT_DIR ?? DEFAULT_SCRIPT_DIR
const REPO_ROOT = resolve(new URL("../../..", import.meta.url).pathname)
const ENV_FILE = process.env.MVP_LIVE_READINESS_ENV_FILE
  ? resolve(process.cwd(), process.env.MVP_LIVE_READINESS_ENV_FILE)
  : join(REPO_ROOT, ".env")
const IS_PRODUCTION_ENV_TARGET = Boolean(
  process.env.MVP_LIVE_READINESS_ENV_FILE,
)
...
console.log("Mobile Retail Ops MVP live readiness")
console.log(
  "This check is non-mutating. It only verifies external/live configuration gates.",
)
...
if (failedChecks.length > 0) {
  ...
  console.error(`Failing live gates: ${failedChecks.join(", ")}`)
  console.error(
    `Run bun run --cwd apps/mobile qa:mvp-live-env-checklist${COMMAND_SUFFIX} for the missing setup keys.`,
  )
  printRequiredMissingSummary()
...
console.log(
  "The remaining full-release gate is bun run --cwd apps/mobile qa:mvp-readiness.",
)
...
function printRequiredMissingSummary() {
